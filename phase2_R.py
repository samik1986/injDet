...
def Injection_Detection_SNAKE_R(img_path):
    img = cv2.imread(img_path, -1)
    basename = os.path.basename(img_path)
    print basename
    f = open(save_path + '/Metadata_R/' + basename +'.txt')
    lines = f.readlines()
    ...
    #x = 4000 + 2000*np.cos(s)
    #y = 4000 + 2000*np.sin(s)
    #init = np.array([x, y]).T
    ...
    pickle.dump(init, open(save_path + '/InjectionSite_SNAKE_datapoints_R/' + basename + '_init_R.p','wb'))
    pickle.dump(snake,open(save_path + '/InjectionSite_SNAKE_datapoints_R/' + basename + '_snake_R.p','wb'))

def Injection_Detect_Pipeline(PMD_path, color = 'GRN'):
    
    ###find the possible list of injection images:
    print 'Finding the interesting images stack - R'
